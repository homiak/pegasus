...
local function get_sightline(pos1, pos2)
	...
	for i = 0, dist do
		...
		if pegasus.get_node_def(pos).walkable then
			return false
		end
	end
	return true
...
function mob:get_wander_pos(min_range, max_range, dir)
	...
	if pegasus.get_node_def(pos).walkable then -- Occurs if small mob is touching a fence
		...
		pos = pegasus.get_ground_level(pos, 1)
	end
	...
	if pegasus.get_node_def(pos2).walkable
	and not dir then
		for _ = 1, 3 do
			move_dir = {
				x = move_dir.z,
			...
			if not pegasus.get_node_def(pos2).walkable then
				break
			end
		end
	elseif dir then
	...
	for i = 1, outset do
		...
		if pegasus.get_node_def(a_pos).walkable
		or not pegasus.get_node_def(b_pos).walkable then
			a_pos = pegasus.get_ground_level(a_pos, floor(self.stepheight or 1))
		end
		if not pegasus.get_node_def(a_pos).walkable then
			pos2 = a_pos
		else
			break
		end
...
function mob:get_wander_pos_3d(min_range, max_range, dir, vert_bias)
	...
	for i = 1, outset do
		local a_pos = vec_add(pos2, vec_multi(move_dir, i))
		if pegasus.get_node_def(a_pos).walkable then
			a_pos = pegasus.get_ground_level(a_pos, floor(self.stepheight or 1))
		end
		if not pegasus.get_node_def(a_pos).walkable then
			pos2 = a_pos
...
function mob:is_pos_safe(pos, ignore_liquid)
	if not pos then return end
	local n_def = pegasus.get_node_def(pos)
	if minetest.get_item_group(n_def.name, "igniter") > 0
	or (not ignore_liquid
	and (n_def.drawtype == "liquid"
	or pegasus.get_node_def(vec_raise(pos, -1)).drawtype == "liquid")) then return false end
	...
	if self.max_fall ~= 0 then
		for _ = 1, self.max_fall or 3 do
			...
			if pegasus.get_node_def(fall_pos).walkable then
				fall_safe = true
				break
			end
		end
...
function mob:get_target(target)
	local alive = pegasus.is_alive(target)
	if not alive then
...
function mob:initiate_utility(utility, ...)
	local func = pegasus.registered_utilities[utility]
...
function mob:_physics()
	-- Physics
	pegasus.default_water_physics(self)
...
function mob:move_to(goal, method, speed_factor)
    ...
    -- Get movement method
    local get_method = pegasus.registered_movement_methods[method]
...
function pegasus.register_mob(name, def)
	...
	def._vitals = def._vitals or pegasus.default_vitals
