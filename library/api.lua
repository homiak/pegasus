...
pegasus.registered_movement_methods = {}

function pegasus.register_movement_method(name, func)
	pegasus.registered_movement_methods[name] = func
end

pegasus.registered_utilities = {}

function pegasus.register_utility(name, func)
	pegasus.registered_utilities[name] = func
end
...
function pegasus.is_alive(mob)
	if not pegasus.is_valid(mob) then
		return false
	end
...
function pegasus.get_node_height_from_def(name)
...
function pegasus.get_node_def(node) -- Node can be name or pos
	if type(node) == "table" then
		node = get_node(node).name
	end
	local def = minetest.registered_nodes[node] or default_node_def
	if def.walkable
	and pegasus.get_node_height_from_def(node) < 0.26 then
...
function pegasus.get_ground_level(pos, range)
	range = range or 2
...
function pegasus.is_pos_moveable(pos, width, height)
	local edge1 = {
		x = pos.x - (width + 0.2),
		y = pos.y,
		z = pos.z - (width + 0.2),
		...
		for x = edge1.x, edge2.x do
			...
			for pointed_thing in ray do
				if pointed_thing.type == "node" then
					...
					if pegasus.get_node_def(name).walkable then
						return false
					end
				end
			end
...
function pegasus.is_blocked(pos, width, height)
	if width <= 0.5 then
...
function pegasus.fast_ray_sight(pos1, pos2, water)
	local ray = minetest.raycast(pos1, pos2, false, water or false)
	local pointed_thing = ray:next()
	while pointed_thing do
		if pointed_thing.type == "node"
		and pegasus.get_node_def(pointed_thing.under).walkable then
...
local fast_ray_sight = pegasus.fast_ray_sight

function pegasus.sensor_floor(self, range, water)
...
function pegasus.sensor_ceil(self, range, water)
...
function pegasus.get_nearby_player(self, range)
	local pos = self.object:get_pos()
	...
	for _, object in ipairs(objects) do
		if object:is_player()
		and pegasus.is_alive(object) then
			return object
		end
	end
end

function pegasus.get_nearby_players(self, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
function pegasus.get_nearby_object(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
	for _, object in ipairs(objects) do
		local ent = pegasus.is_alive(object) and object:get_luaentity()
		if ent
...
function pegasus.get_nearby_objects(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
pegasus.get_nearby_entity = pegasus.get_nearby_object
pegasus.get_nearby_entities = pegasus.get_nearby_objects

--------------------
-- Global Mob API --
--------------------

function pegasus.default_water_physics(self)
...
function pegasus.default_vitals(self)
	local pos = self.stand_pos
...
function pegasus.drop_items(self)
...
function pegasus.basic_punch_func(self, puncher, tflp, tool_caps, dir)
...
function pegasus.mob_follow(self, player)
...
function pegasus.mob_stay(self)
    self.object:set_velocity({x=0, y=0, z=0})
end

function pegasus.mob_wander(self, dtime)
