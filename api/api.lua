...
function pegasus.head_tracking(self)
	...
	and random(4) < 2 then
		...
		local players = mobforge.get_nearby_players(self, 6)
...
function pegasus.get_attack_score(entity, attack_list)
	...
	local target = entity._target or (entity.attacks_players and mobforge.get_nearby_player(entity))
	...
	and minetest.is_creative_enabled(target:get_player_name())) then
		target = mobforge.get_nearby_object(entity, attack_list)
...
function pegasus.add_break_particle(pos)
	pos = vec_round(pos)
	local def = mobforge.get_node_def(pos)
		...
		if feed_no >= 5 then
			...
			if tame then
				...
				minetest.add_particlespawner({
					...
					size = {min = 4, max = 6},
					glow = 16,
					texture = "mobforge_particle_green.png"
				})
			end
			if breed then
...
function pegasus.mount(self, player, params)
	if not mobforge.is_alive(player)
...
function pegasus.punch(self, puncher, ...)
	...
	mobforge.basic_punch_func(self, puncher, ...)
