...
modding.register_abm_spawn("pegasus:pegasus", {
...
minetest.register_abm({
	...
	action = function(pos, _, active_object_count)
		...
		local spawn_definitions = modding.registered_mob_spawns
		...
		if #spawnable_mobs > 0 then
			...
			local spawn_definition = modding.registered_mob_spawns[mob_to_spawn]
			...
			if group_size > 1 then
				...
				for _ = 1, group_size do
					...
					if not modding.is_pos_moveable(spawn_pos, 0.5, 0.5) then
						spawn_pos = pos
					end
