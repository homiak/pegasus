...
function waterdragon.rare_water_breath(self, pos2)
	...
	if breath_delay <= 0 then
		...
		for i = 1, 32, spread do
			...
			freeze_nodes(rare_water_pos, spread)
			if random(5) < 2 then
...
minetest.register_on_mods_loaded(function()
	...
	for k, v in pairs(waterdragon.wyvern_api) do
		minetest.registered_entities["waterdragon:jungle_wyvern"][k] = v
	end
end)
	...
	for name, def in pairs(minetest.registered_entities) do
		...
		or minetest.registered_entities[name]._creatura_mob then
			...
			local on_punch = function(self, puncher, time_from_last_punch, tool_capabilities, direction, damage)
				...
				for object, data in pairs(waterdragon.wyverns) do
					if object
					and object:get_pos()
					and data.owner
...
function waterdragon.wyvern_step(self, dtime)
	-- Animation Tracking
	local current_anim = self._anim
	...
	if not waterdragon.wyverns[self.object] then
		waterdragon.wyverns[self.object] = {owner = self.owner}
	end
end
...
function waterdragon.wyvern_rightclick(self, clicker)
