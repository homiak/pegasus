...
--[[local function debugpart(pos, time, tex)
	minetest.add_particle({
		pos = pos,
		texture = tex or "mobstogo_particle_red.png",
...
local get_node_def = mobstogo.get_node_def
--local get_node_height = mobstogo.get_node_height_from_def

function mobstogo.get_collision(self, dir, range)
...
mobstogo.get_collision_ranged = mobstogo.get_collision

local get_collision = mobstogo.get_collision
...
function mobstogo.get_avoidance_lift(self, pos2, range)
	...
	for i = 1, range, 0.5 do -- 0.5 increment increases accuracy
		...
		def = mobstogo.get_node_def(check_pos)
		if not ceil_pos
		and (def.walkable
...
function mobstogo.get_avoidance_lift_aquatic(self, pos2, range)
	range = ceil(max(range or 1, 0.5))
	local height_half = (self.height or 1) * 0.5
	local center_y = pos2.y + height_half
	local check_pos = {x = pos2.x, y = center_y, z = pos2.z}
	...
	for i = 1, range, 0.5 do -- 0.5 increment increases accuracy
		...
		check_pos.y = center_y + i
		if not ceil_pos
		and minetest.get_item_group(mobstogo.get_node_def(check_pos).name, "liquid") < 1 then
			ceil_pos = check_pos
		end
		check_pos.y = center_y - i
		if not floor_pos
...
function mobstogo.get_context_default(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local width, height = self.width or 0.5, self.height or 1
	...
	and mobstogo.get_node_def(pointed.under).walkable then
...
function mobstogo.get_context_large(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local width, height = self.width or 0.5, self.height or 1
	local y_offset = math.min(self.stepheight or height)
	pos.y = pos.y + y_offset
	local collision = mobstogo.get_collision(self, steer_dir, range)
...
function mobstogo.get_context_small(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
...
function mobstogo.get_context_small_aquatic(self, goal, steer_dir, interest, danger, range)
	local pos = self.object:get_pos()
	if not pos then return end
	pos = vector.round(pos)
	local width = self.width or 0.5
...
function mobstogo.calc_steering(self, goal, get_context, range)
	if not goal then return end
	get_context = get_context or mobstogo.get_context_default
	local pos, yaw = self.object:get_pos(), self.object:get_yaw()
...
function mobstogo.get_context_steering(self, goal, range, water)
	local context = mobstogo.get_context_default
	local width, height = self.width, self.height
	if width > 0.5
	or height > 1 then
		context = mobstogo.get_context_large
	elseif water then
		context = mobstogo.get_context_small_aquatic
	end
	return mobstogo.calc_steering(self, goal, context, range)
end
...
function mobstogo.action_move(self, pos2, timeout, method, speed_factor, anim)
	...
	local function func(_self)
		...
		if timer <= 0
		or not safe
		or _self:move_to(pos2, method or "mobstogo:obstacle_avoidance", speed_factor or 0.5) then
			return true
		end
	end
...
mobstogo.action_walk = mobstogo.action_move -- Support for outdated versions

-- Idle

function mobstogo.action_idle(self, time, anim)
	local timer = time
	local function func(_self)
...
function mobstogo.action_fallover(self)
	local zrot = 0
	local init = false
	local dir = 1
...
mobstogo.register_movement_method("mobstogo:pathfind_theta", function(self)
	local path = {}
	local steer_to
	local steer_int = 0
	...
	local function func(_self, goal, speed_factor)
		...
		steer_to = path_dir or (steer_int <= 0 and mobstogo.calc_steering(_self, goal)) or steer_to

		path = (#path > 0 and path) or (mobstogo.pathfinder.find_path_theta(_self, pos, goal) or {})
...
mobstogo.register_movement_method("mobstogo:pathfind", function(self)
	local path = {}
	local steer_to
	local steer_int = 0
	local arrival_threshold = clamp(self.width, 0.5, 1)
	...
	local function func(_self, goal, speed_factor)
		...
		path = (#path > 0 and path) or (mobstogo.pathfinder.find_path(_self, pos, goal) or {})
...
mobstogo.register_movement_method("mobstogo:steer_small", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		...
		local speed = abs(_self.speed or 2) * speed_factor or 0.5
		steer_int = (steer_int > 0 and steer_int - _self.dtime) or 1 / math.max(speed, 1)
		steer_to = (steer_int <= 0 and mobstogo.calc_steering(_self, goal)) or steer_to
		-- Apply Movement
		_self:turn_to(dir2yaw(steer_to or vec_dir(pos, goal)), turn_rate)
		_self:set_forward_velocity(speed)
	end
...
mobstogo.register_movement_method("mobstogo:steer_large", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		...
		local speed = abs(_self.speed or 2) * speed_factor or 0.5
		steer_int = (steer_int > 0 and steer_int - _self.dtime) or 1 / math.max(speed, 1)
		steer_to = (steer_int <= 0 and mobstogo.calc_steering(_self, goal, mobstogo.get_context_large)) or steer_to
		-- Apply Movement
		_self:turn_to(dir2yaw(steer_to or vec_dir(pos, goal)), turn_rate)
		_self:set_forward_velocity(speed)
	end
...
mobstogo.register_movement_method("mobstogo:walk_simple", function(self)
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
		local pos = _self.object:get_pos()
		if not pos or not goal then return end
...
mobstogo.register_movement_method("mobstogo:context_based_steering", function(self)
	local steer_to
	local steer_int = 0
	self:set_gravity(-9.8)
	local function func(_self, goal, speed_factor)
...
mobstogo.register_movement_method("mobstogo:obstacle_avoidance", function(self)
