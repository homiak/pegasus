...
mobstogo.registered_mob_spawns = {}
mobstogo.registered_on_spawns = {}
	...
	for y = 0, height, height / ceil(height) do
		for z = -width, width, w_iter do
			for x = -width, width, w_iter do
				...
				local def = mobstogo.get_node_def(pos2)
...
local function do_on_spawn(pos, obj)
	...
	local spawn_functions = mobstogo.registered_on_spawns[name] or {}
...
function mobstogo.register_spawn_item(name, def)
	...
	or (def.hex_primary and def.hex_secondary)) then
		...
		local base = "(mobstogo_spawning_crystal_primary.png^[multiply:#" .. primary .. ")"
		local spots = "(mobstogo_spawning_crystal_secondary.png^[multiply:#" .. secondary .. ")"
...
function mobstogo.register_on_spawn(name, func)
	if not mobstogo.registered_on_spawns[name] then
		mobstogo.registered_on_spawns[name] = {}
	end
	table.insert(mobstogo.registered_on_spawns[name], func)
end
...
--[[mobstogo.register_abm_spawn("mymod:mymob", {
	chance = 3000,
	interval = 30,
	min_height = 0,
	max_height = 128,
...
local protected_spawn = minetest.settings:get_bool("mobstogo_protected_spawn", true)
local abr = (tonumber(minetest.get_mapgen_setting("active_block_range")) or 4) * 16
local max_per_block = tonumber(minetest.settings:get("mobstogo_mapblock_limit")) or 12
local max_in_abr = tonumber(minetest.settings:get("mobstogo_abr_limit")) or 24
local min_abm_dist = min(abr / 2, tonumber(minetest.settings:get("mobstogo_min_abm_dist")) or 32)

local mobs_spawn = minetest.settings:get_bool("mobs_spawn") ~= false

local mapgen_mobs = {}

function mobstogo.register_abm_spawn(mob, def)
	...
	local function spawn_func(pos, aocw)
		...
		if group_size > 1 then
			...
			for _ = 1, group_size do
				...
				spawn_pos = mobstogo.get_ground_level({
		...
		minetest.log("action",
			"[mobstogo] [ABM Spawning] Spawned " .. group_size .. " " .. mob .. " at " .. minetest.pos_to_string(pos))
	...
	mobstogo.registered_mob_spawns[mob] = {
...
minetest.register_node("mobstogo:spawn_node", {
	drawtype = "airlike",
...
local mapgen_spawning_int = tonumber(minetest.settings:get("mobstogo_mapgen_spawn_interval")) or 64

if mapgen_spawning then
	local chunk_delay = 0
	local c_air = minetest.get_content_id("air")
	local c_spawn = minetest.get_content_id("mobstogo:spawn_node")
	...
	minetest.register_on_generated(function(minp, maxp)
		...
		for _, mob_name in ipairs(mapgen_mobs) do
			...
			def = mobstogo.registered_mob_spawns[mob_name]
	...
	local spawn_interval = tonumber(minetest.settings:get("mobstogo_spawn_interval")) or 10

	minetest.register_abm({
		label = "mobstogo Spawning",
		nodenames = {"mobstogo:spawn_node"},
		...
		action = function(pos)
			...
			if amount > 0 then
				...
				minetest.log("action",
					"[mobstogo] Spawned " .. amount .. " " .. name .. " at " .. minetest.pos_to_string(pos))
			else
				obj = minetest.add_entity(pos, name)
				do_on_spawn(pos, obj)
				minetest.log("action",
					"[mobstogo] Spawned a " .. name .. " at " .. minetest.pos_to_string(pos))
			end
...
function mobstogo.register_mob_spawn(name, def)
	local spawn_def = {
	...
	--mobstogo.registered_mob_spawns[name] = spawn_def

	mobstogo.register_abm_spawn(name, spawn_def)
end

function mobstogo.register_spawn_egg(name, col1, col2, inventory_image)
	if col1 and col2 then
		local base = "(mobstogo_spawning_crystal_primary.png^[multiply:#" .. col1 .. ")"
		local spots = "(mobstogo_spawning_crystal_secondary.png^[multiply:#" .. col2 .. ")"
		inventory_image = base .. "^" .. spots
	end
	local mod_name = name:split(":")[1]
	local mob_name = name:split(":")[2]
