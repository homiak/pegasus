...
mobstogo.registered_movement_methods = {}

function mobstogo.register_movement_method(name, func)
	mobstogo.registered_movement_methods[name] = func
end

mobstogo.registered_utilities = {}

function mobstogo.register_utility(name, func)
	mobstogo.registered_utilities[name] = func
end
...
function mobstogo.is_valid(mob)
...
function mobstogo.is_alive(mob)
	if not mobstogo.is_valid(mob) then
		return false
	end
...
function mobstogo.get_node_height_from_def(name)
...
function mobstogo.get_node_def(node) -- Node can be name or pos
	if type(node) == "table" then
		node = get_node(node).name
	end
	local def = minetest.registered_nodes[node] or default_node_def
	if def.walkable
	and mobstogo.get_node_height_from_def(node) < 0.26 then
...
function mobstogo.get_ground_level(pos, range)
	range = range or 2
...
function mobstogo.is_pos_moveable(pos, width, height)
	local edge1 = {
		x = pos.x - (width + 0.2),
		y = pos.y,
		z = pos.z - (width + 0.2),
		...
		for x = edge1.x, edge2.x do
			...
			for pointed_thing in ray do
				if pointed_thing.type == "node" then
					...
					if mobstogo.get_node_def(name).walkable then
						return false
					end
				end
			end
...
function mobstogo.is_blocked(pos, width, height)
	if width <= 0.5 then
...
function mobstogo.fast_ray_sight(pos1, pos2, water)
	local ray = minetest.raycast(pos1, pos2, false, water or false)
	local pointed_thing = ray:next()
	while pointed_thing do
		if pointed_thing.type == "node"
		and mobstogo.get_node_def(pointed_thing.under).walkable then
...
local fast_ray_sight = mobstogo.fast_ray_sight

function mobstogo.sensor_floor(self, range, water)
...
function mobstogo.sensor_ceil(self, range, water)
...
function mobstogo.get_nearby_player(self, range)
	local pos = self.object:get_pos()
	...
	for _, object in ipairs(objects) do
		if object:is_player()
		and mobstogo.is_alive(object) then
			return object
		end
	end
end

function mobstogo.get_nearby_players(self, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
function mobstogo.get_nearby_object(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
	for _, object in ipairs(objects) do
		local ent = mobstogo.is_alive(object) and object:get_luaentity()
		if ent
...
function mobstogo.get_nearby_objects(self, name, range)
	local pos = self.object:get_pos()
	if not pos then return end
	local stored = self._nearby_obj or {}
	local objects = (#stored > 0 and stored) or self:store_nearby_objects(range)
...
mobstogo.get_nearby_entity = mobstogo.get_nearby_object
mobstogo.get_nearby_entities = mobstogo.get_nearby_objects

--------------------
-- Global Mob API --
--------------------

function mobstogo.default_water_physics(self)
...
function mobstogo.default_vitals(self)
	local pos = self.stand_pos
...
function mobstogo.drop_items(self)
...
function mobstogo.basic_punch_func(self, puncher, tflp, tool_caps, dir)
...
local path = minetest.get_modpath("mobstogo")
