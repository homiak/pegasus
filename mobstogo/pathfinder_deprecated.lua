...
local a_star_alloted_time = tonumber(minetest.settings:get("mobstogo_a_star_alloted_time")) or 500
local theta_star_alloted_time = tonumber(minetest.settings:get("mobstogo_theta_star_alloted_time")) or 700
...
local moveable = mobstogo.is_pos_moveable
...
local function is_on_ground(pos)
	...
	if mobstogo.get_node_def(ground).walkable then
		return true
	end
	return false
end
...
local function get_line_of_sight(a, b)
	...
	if #line < 1 then
		...
		for i = 1, #line do
			...
			if mobstogo.get_node_def(node.name).walkable then
				return false
			end
		end
	end
...
local function is_pos_moveable_vm(pos, width, height, area, data)
	...
	for x = pos1.x, pos2.x do
		...
		if c ~= c_air then
			...
			if mobstogo.get_node_def(c_name).walkable then
				return false
			end
		end
	end
...
function mobstogo.find_lvm_path(self, start, goal, obj_width, obj_height, max_open, climb, fly, swim)
	...
	local function get_neighbors(pos, width, height, tbl, open, closed, vm_area, vm_data)
		...
		for i = 1, #tbl do
			...
			if can_move then
				...
				if not fly and not swim then
					if not can_move then -- Step Up
						...
						local step = mobstogo.get_ground_level(vector.new(neighbor), 1)
			...
			if can_move
			and (not swim
			or mobstogo.get_node_def(neighbor).drawtype == "liquid") then
...
function mobstogo.find_path(self, start, goal, obj_width, obj_height, max_open, climb, fly, swim)
	climb = climb or false
	fly = fly or false
	swim = swim or false
...
function mobstogo.find_theta_path(self, start, goal, obj_width, obj_height, max_open, climb, fly, swim)
	climb = climb or false
	fly = fly or false
	swim = swim or false
	...
	local function get_neighbors(pos, width, height, tbl, open, closed)
		...
		for i = 1, #tbl do
			...
			and not fly
			and not swim then
				neighbor = mobstogo.get_ground_level(neighbor, 1)
			end
